...
function WeeklyChart({ data }) {
  ...
  return <div className="flex h-36 items-end gap-2 border-b border-[#3b4049] pb-1">
    {data.map((item) => <div className="flex min-w-0 flex-1 flex-col items-center justify-end gap-1" key={item.label}>
      <span className="text-xs font-bold text-white">{item.value || ""}</span>
      <div className="w-full max-w-10 rounded-t bg-[#e5253b]" style={{ height: `${Math.max((item.value / max) * 90, item.value ? 8 : 2)}px`, opacity: item.value ? 1 : 0.2 }} />
      <span className="hidden text-[10px] text-[#aeb2ba] sm:block">{item.label}</span>
    </div>)}
  </div>
}
...
export default function ProgressReport({ report }) {
  ...
  return <div className="grid gap-5">
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-4">
      <div className={card}><p className="mb-2 text-xs font-bold uppercase text-[#aeb2ba]">Entrenamientos</p><strong className="text-3xl text-[#e5253b]">{summary.completed_workouts}</strong></div>
      <div className={card}><p className="mb-2 text-xs font-bold uppercase text-[#aeb2ba]">Volumen total</p><strong className="text-2xl text-white">{numberText(summary.total_volume_lb)}</strong><span className="ml-1 text-xs text-[#aeb2ba]">lb</span></div>
